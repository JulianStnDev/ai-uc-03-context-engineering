...
Alle Varianten nutzen denselben System-Prompt und dasselbe Kontextformat, in dem
Dateiname, Titel und Datum jeder Quelle sichtbar sind. Ergebnisse werden Zeile für
Zeile an data/answers.jsonl angehängt; bereits beantwortete (Frage, Variante)-Paare
werden beim erneuten Start übersprungen.
"""
import json
import re
import time
...
SYSTEM_PROMPT = """Du bist der Support-Assistent von FocusFlow, einer Habit-Tracker-App. Du beantwortest Kundenfragen ausschließlich anhand der Hilfeartikel im Block <kontext>.
...
def main():
    ...
    if ANSWERS_PATH.exists():
        done = {(r["id"], r["variante"]) for r in map(json.loads, ANSWERS_PATH.open(encoding="utf-8"))}
    ...
    with ANSWERS_PATH.open("a", encoding="utf-8") as out:
        ...
        for variant in VARIANTS:
            for q in goldset:
                ...
                u = response.usage
                row = {
                    "id": q["id"], "variante": variant, "frage": q["frage"], "antwort": answer,
